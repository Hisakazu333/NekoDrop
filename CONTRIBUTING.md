...
本仓库使用 `main / develop / desktop-develop / personal dev branch / topic branch` 流程。
...
personal dev branch
  每个人自己的长期开发分支，例如 dev/hisakazu。日常提交先落到这里，再通过 PR 合到 develop 或 desktop-develop。

topic branch
  一次性短分支。适合风险较高、需要隔离评审或多人协作的单项改动。
```

`develop` 和 `desktop-develop` 都不是个人开发分支。不要把没完成的代码直接推到这两个分支。当前维护者日常开发默认使用 `dev/hisakazu`，后续新成员使用 `dev/<name>`。
...
- 个人长期分支和集成分支不能顺手删除：`dev/<name>`、`develop`、`desktop-develop`、`main` 都要保留
