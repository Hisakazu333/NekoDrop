...
本仓库使用 `develop -> main` 的收口流程。

```text
feature/docs/fix/security branch
  -> PR into develop
  -> develop accumulates verified changes
  -> release/rollup PR from develop into main
  -> tag and package from main
```

`main` 是发布主线，只接收从 `develop` 发起的 release / rollup PR。不要把日常功能 PR 直接合进 `main`。
...
- 日常文档、修复、功能、安全、UI、测试：合到 `develop`
