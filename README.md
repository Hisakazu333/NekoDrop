...
1. bundle 样例和 adapter 规范：给 session、skill、workspace、agent profile 明确导出和导入边界
2. local bridge 真实发送 / 导入执行：本机应用只提交请求，NekoDrop 继续负责确认、传输和落盘
3. local bridge 事件订阅：收到 bundle、传输状态变化和待确认请求可以推给本机应用
