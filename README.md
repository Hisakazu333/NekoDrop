...
| 手动资料包 | 已有创建、发送入口、收到后的暂存查看、删除、过期清理和手动导入到本机导入区 |
| 自动导出 session / skill / workspace | 未接入 |
| 本机接入 local bridge | 已有协议模型、localhost runtime、权限 scope、只读 handler、设置页自测、授权码确认、限时授权持久化、授权列表和撤销 |
| local bridge 真实发送 / 导入执行 | 部分接入；bundle.send 可进入桌面发送主线，bundle.import 可导入本机导入区 |
...
1. local bridge worker 自动化：现在已有授权队列、发送执行入口、导入执行入口和结果查询，后面要把桌面端内部 worker 跑顺
2. local bridge 事件流：现在已有授权后的 `events.poll`，后面再做长连接通知
3. 上层 adapter 最小闭环：让外部应用按通用规范导出、导入 bundle，不把协议写死到某个应用
