...
1. bundle 导入计划和冲突处理：导入前先看会写什么、是否同名、能不能安全导入
2. 上层 adapter 最小闭环：让外部应用按通用规范导出、导入 bundle，不把协议写死到某个应用
3. local bridge 长连接通知：现在已有 `events.poll` 短等待和动作生命周期事件，后面再补真正事件订阅
