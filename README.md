...
| 本机接入 local bridge | 已有协议模型、localhost runtime、权限 scope、只读 handler、设置页自测、授权码确认和限时授权持久化 |
...
1. local bridge 授权管理：列出、撤销、过期清理已授权应用
2. bundle 样例和 adapter 规范：给 session、skill、workspace、agent profile 明确导出和导入边界
3. local bridge 真实发送 / 导入执行：本机应用只提交请求，NekoDrop 继续负责确认、传输和落盘
