...
- 未完成能力必须标记为待接入。
- 连接码只做兜底，主流程是自动发现设备。
- NekoDrop 做文件互传产品，NekoLink 做通信协议，不要混成一团。
- OpenNeko / NekoState 的接口可以预留，但不能在 UI 里伪装成已完成。

## 第三方与许可

第三方依赖和声明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

项目许可证还未最终确定。正式开源前需要补充根目录 `LICENSE`，并明确 NekoDrop、NekoLink、商业资产和 OpenNeko 相关模块的授权边界。
