...
| 本机接入 local bridge | 已有协议模型、权限 scope、只读 handler、设置页自测、授权码确认和进程内授权门控 |
| local bridge localhost runtime / 持久化授权 | 未接入 |
...
1. local bridge localhost runtime：让本机应用通过受控 API 请求发送和导入 bundle，不直接碰网络协议
2. bundle 样例和 adapter 规范：给 session、skill、workspace、agent profile 明确导出和导入边界
3. local bridge 授权持久化：列出、撤销、过期清理已授权应用
