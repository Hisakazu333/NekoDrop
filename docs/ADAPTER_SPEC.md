...
`transactions` 描述真实应用导入时的事务边界。当前要求 adapter 必须支持 dry-run、写 receipt、按 receipt rollback，并且必须处理冲突策略。`migration_policy=manual_only` 表示遇到 schema 迁移时只能提示用户或 adapter 自己确认；`adapter_managed` 表示 adapter 可以处理自己的迁移，但仍要写 receipt，失败时不能留下半成品。不要声明自动覆盖或静默迁移。
...
导入结果会带回 `conflict_strategy`、`skipped_file_count`、`has_import_receipt`、`rollback_file_count` 和 `can_request_rollback`。receipt 路径属于 NekoDrop 本机私有路径，普通 bridge response 不返回；adapter 只根据这些状态字段判断是否能发起 `bundle.rollback`。`bundle.detail` 需要 `bundle.read` 授权，详情响应和列表快照会返回 `import_allowed`、`can_import_now`、`import_blocking_reason`、`import_conflict_count`、`import_conflict_strategies`、`has_import_receipt`、`rollback_file_count`、`can_rollback_now`、`can_request_rollback`、`rolled_back_file_count` 和 `rollback_blocking_reason`，但不返回 `staging_path`、`import_path`、`import_destination` 或 plan 里的本机目标路径。通用 adapter 示例的 `receipt-state` 会把这些字段归纳成 `ready_to_import`、`import_conflict`、`imported_can_rollback`、`imported_no_rollback`、`rolled_back`、`save_only`、`not_imported` 或 `missing`，并给出示例层的 `next_action`。receipt 记录目标目录、实际导入和跳过的 payload 路径。NekoDrop 可以基于 receipt 生成回滚计划，也可以执行 `bundle.rollback`。这个撤回只删除 NekoDrop 本机导入区里“本次导入记录”对应的文件；`skip_conflicts` 跳过的既有文件不会被删除，也不会写回第三方应用目录。adapter 应把这些结果交给用户或自己的导入流程处理，不能静默覆盖。
