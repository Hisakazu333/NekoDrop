...
| 能力 | 为什么需要 | 当前策略 |
| --- | --- | --- |
| 加密文件流 | bundle 可能包含 session、workspace 或 agent profile，payload 不能长期依赖明文 TCP | encrypted session 路径已有加密 file frames；敏感 bundle 进入主流程前还要完成接收端 streaming 解密和长期身份认证 |
...
接收端只能把 bundle 保存到应用控制的 staging 目录。导入动作由上层应用读取 staging 目录后完成。
...
- 敏感 bundle 进入默认主流程前，需要完成 streaming 解密、长期身份认证和导入确认
