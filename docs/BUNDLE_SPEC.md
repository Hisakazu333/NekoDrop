...
桌面端现在有一个内部 handler，可以处理 `devices.list`、`bundle.detail` 和 `transfer.status` 的只读快照。`authorization.request` 会返回申请的 scope、reason、ttl 和短授权码；设置页可以确认授权码，进程内 runtime 会记录该 client 的临时权限，并让已授权 client 通过 `bundle.send` / `bundle.import` 的门控。但真实发送和导入 runtime 还没有开放给 bridge。响应会标记 `read_only`、`requires_user_confirmation` 或 `authorized`。它不是公开 localhost 服务，也不会绕过用户确认去发送或导入 bundle。
