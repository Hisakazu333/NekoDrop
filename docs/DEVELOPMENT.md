...
当前还没有接入加密文件流、replay window、长期身份密钥认证、iroh 真实运行时、Relay / P2P、手机端互传主流程和 OpenNeko 支撑层。界面和文档应将这些能力标记为规划中或实验中，不应把占位数据描述为真实桌面能力。
...
5. 完成 encrypted session control 后续安全项：replay window、长期身份密钥认证、文件流加密。
6. 定义 NekoLink bundle，为 skills、session、workspace、agent profile 提供统一包格式。
7. 接 CCS/OpenNeko 本地桥，再做 iroh / Relay / P2P transport 验证。
