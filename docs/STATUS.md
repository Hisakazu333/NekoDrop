...
| Encrypted session handshake | 部分接入 | 协议层已有 `session.hello` / `session.ready` payload、帧读写、cipher 协商 helper、算法标签白名单、transcript hash、`session.ready` 构造器、网络层 verified reader、X25519 ephemeral keypair / shared secret API、HKDF-SHA256 send/receive traffic key material 派生；还没有把控制消息加密、文件流加密接入传输主线。 |
...
V0.8
  加密 session：
  补 nonce / frame 计数、AEAD 控制帧和文件流加密，让文件 offer 和传输消息进入可信加密通道。
```
