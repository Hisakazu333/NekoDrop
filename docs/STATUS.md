...
| Encrypted session handshake | 部分接入 | 协议层已有 `session.hello` / `session.ready` payload、帧读写、cipher 协商 helper、transcript hash、`session.ready` 构造器和网络层 verified reader；还没有把真实密钥交换、控制消息加密、文件流加密接入传输主线。 |
