...
- 点附近设备发送。
- 连接码兜底。
- IP 过滤，避免把 `127.0.0.1`、`198.18.x.x` 这类地址交给另一台电脑。

剩余问题：

- 公司、校园、公共 Wi-Fi 可能屏蔽 mDNS。
- 有线台式机和无线笔记本可能不在同一广播域。
- Windows 防火墙仍可能阻止接收端口。
- 连接码仍然只是兜底，不适合做主体验。
- iroh / Relay / P2P 还没有接入，不能解决所有非局域网场景。
...
## 3. 当前方案：mDNS Discovery，连接码兜底

当前主线是 mDNS / DNS-SD 发现，流程参考 LocalSend 这类局域网互传产品，但不做协议兼容承诺。
...
V0.6 发现层基础已经接入，后续完成标准是稳定性和错误恢复：
...
- 有线 + 无线不同网段时，明确提示用户使用连接码、Relay 或后续 iroh 方案。
